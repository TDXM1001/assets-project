...
Status: DRAFT
Mode: Builder
Supersedes: 资产管理系统-业务流程演练稿.md

## 目标

这份文档把前面的 SQL 设计推进到“接近可执行”的层级，固定三类内容：
...
1. 第一阶段不加数据库强外键。
2. 仍然只覆盖 MVP 的 9 张业务表。
3. 审计字段统一保留：`create_by/create_time/update_by/update_time/remark/del_flag`
...
1. `serial_no` 先只建普通索引，不做唯一约束
2. `before_snapshot/after_snapshot` 用 `longtext`
3. 正式建表前，先再过一遍字段命名和字典值

## Next Steps

1. 基于这份草案输出真正的 `.sql` 文件
2. 再补 `sys_menu` 初始化 SQL 草案
