-- ----------------------------
-- 资产台账新增页面增量脚本
-- Date: 2026-03-27
-- Scope: sys_menu / sys_role_menu
-- 说明：
-- 1. 资产台账从弹窗新增升级为独立页面。
-- 2. 仍然复用现有 asset:info:add 权限，不新增 permission 字符串。
-- ----------------------------

select @asset_info_menu_id := menu_id
from sys_menu
where parent_id = (
  select menu_id
  from sys_menu
  where parent_id = 0 and path = 'asset'
  limit 1
)
  and path = 'info'
limit 1;

-- 独立新增页：隐藏在菜单树中，只作为路由入口，不占左侧菜单位
insert into sys_menu (
  menu_name, parent_id, order_num, path, component, `query`, route_name,
  is_frame, is_cache, menu_type, visible, status, perms, icon,
  create_by, create_time, update_by, update_time, remark
)
select
  '新增资产', @asset_info_menu_id, 9, 'create', 'asset/info/create/index', '', 'AssetInfoCreate',
  1, 0, 'C', '1', '0', 'asset:info:add', 'edit',
  'admin', sysdate(), '', null, '资产台账新增页面'
where @asset_info_menu_id is not null
  and not exists (
    select 1 from sys_menu where parent_id = @asset_info_menu_id and path = 'create'
  );

select @asset_info_create_menu_id := menu_id
from sys_menu
where parent_id = @asset_info_menu_id and path = 'create'
limit 1;

select @asset_admin_role_id := role_id
from sys_role
where role_key = 'asset_admin'
limit 1;

-- 资产管理员：新增页面可直接进入，仍然沿用 asset:info:add 权限
insert into sys_role_menu (role_id, menu_id)
select @asset_admin_role_id, @asset_info_create_menu_id
from dual
where @asset_info_create_menu_id is not null
  and not exists (
    select 1 from sys_role_menu rm
    where rm.role_id = @asset_admin_role_id
      and rm.menu_id = @asset_info_create_menu_id
  );
